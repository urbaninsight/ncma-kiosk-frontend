...
export default function AdditionalControls() {
  ...
  return (
    <div className="cloverSm:flex-row cloverSm:gap-x-[0.618rem] mr-[-1px] flex flex-col items-center justify-center gap-y-[0.618rem] rounded-lg p-4">
      {museumObjectState.kioskMode && (
        <div className="additional-controls-instruction w-max-content cloverSm:flex hidden h-8 items-center justify-center rounded-md bg-black px-[18px] py-[15px] text-white">
      ...
      <button
        className="additional-controls-button w-max-content cloverSm:w-auto cloverSm:px-[18px] group flex h-11 w-11 items-center justify-center rounded-full border-2 border-white bg-black py-[15px] text-white hover:border-ncmaOrange hover:bg-ncmaDarkOrange hover:text-ncmaOrange"
        onPointerUp={onLearnMoreClick}
      >
        <span className="cloverSm:mr-4 text-lg">
          <InfoSpeechBubbleIcon
            className="h-5 w-5 fill-white group-hover:fill-ncmaOrange"
            fill="inherit"
          />
        </span>
        <span className="cloverSm:block hidden">
